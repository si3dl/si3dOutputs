    ...
    if (st3d == 0) || (stpl == 0) || (sttr == 0)
        ...
        if (count == 1)
            if (iTurb == 1)
                ...
                T = out_array3D(8:13:length(out_array3D)-5);
                Dv = out_array3D(9:13:length(out_array3D)-4);
            ...
            elseif (iTurb == 0)
                ...
                % h = out_array3D(4:8:length(out_array3D)-4);
            ...
            if nTracer > 0
                tracerc = NaN(ipointsTr(1),nTracer);
                for tr = 1:nTracer
                    out_arrayTr=fread(fidTr(tr),4*ipointsTr(tr),'float32');
                    dumTrx = out_arrayTr(1:4:length(out_arrayTr)-3);
                    dumTry = out_arrayTr(2:4:length(out_arrayTr)-2);
                    dumTrz = out_arrayTr(3:4:length(out_arrayTr)-1);
                    tracerc(:,tr)=out_arrayTr(4:4:length(out_arrayTr));
                end
            end
        else
            if (iTurb == 1)
                ...
                T = out_array3D(5:10:length(out_array3D)-5);
                Dv = out_array3D(6:10:length(out_array3D)-4);
            ...
            elseif (iTurb == 0)
                ...
                % h = out_array3D(1:5:length(out_array3D)-4);
            ...
            if nTracer > 0
                for tr = 1:nTracer
                    out_arrayTr=fread(fidTr(tr),1*ipointsTr(tr),'float32');
                    tracerc(:,tr)=out_arrayTr(1:1:length(out_arrayTr));
                end
            end
        end
        ...
        if iTurb == 1
            ...
            vtkwriteSV_v1([outputFile,'_',num2str(round(istep(count)*dt/3600, 1)),'.vtk'],...
        ...
        if count == n_frames
            fprintf(fidPV,'%s\n',['		{ "name" : "',outputFile,'_',num2str(round(istep(count)*dt/3600,1)),'.vtk",',' "time" : ',num2str((istep(count))*dt),'}']);
        else
            fprintf(fidPV,'%s\n',['		{ "name" : "',outputFile,'_',num2str(round(istep(count)*dt/3600,1)),'.vtk",',' "time" : ',num2str((istep(count))*dt),'},']);
        end
